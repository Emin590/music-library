...
@section('content')
        <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
            <header class="bg-zinc-900 border-b border-zinc-800 p-4 flex justify-end items-center gap-4">
                ...
                @auth
                    <div class="flex items-center gap-4">
                        ...
                        <form action="/logout" method="POST">
                            @csrf
                            <button type="submit" class="text-sm text-zinc-400 hover:text-white">
                                Log Out
                            </button>
                        </form>
                    </div>
                ...
                @guest
                    <a href="/register" class="bg-white hover:bg-gray-200 text-black font-bold py-2 px-4 rounded-full transition transform hover:scale-105">
                        Register
                    </a>
                    <a href="/login" class="bg-white hover:bg-gray-200 text-black font-bold py-2 px-4 rounded-full transition transform hover:scale-105">
                        Login
                    </a>
                @endguest
            </header>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    <div class="bg-zinc-800 p-4 rounded-lg hover:bg-zinc-700 transition group relative">
                        <div class="relative aspect-square mb-4 bg-black rounded-md overflow-hidden shadow-lg">
                            <img src="{{ $song->album_cover ?? 'https://placehold.co/300x300' }}" alt="Album Art" class="object-cover w-full h-full">
                        </div>
                        
                        <h4 class="font-bold truncate text-white">{{ $song->title }}</h4>
                        <p class="text-sm text-zinc-400 truncate mb-4">{{ $song->artist }}</p>

                        <div class="flex justify-end items-center border-t border-zinc-600 pt-3 mt-2">
                            
                            <a href="/{{ $song->id }}" class="text-zinc-400 hover:text-white transition" title="View Details">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                            </a>

                            <a href="/{{ $song->id }}/edit" class="text-blue-400 hover:text-blue-300 transition" title="Edit">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>

                            <form action="/{{ $song->id }}/delete" method="POST" onsubmit="return confirm('Are you sure?');">
                                @csrf
                                @method('DELETE')
            ...
            <div class="h-16 bg-zinc-900 border-t border-zinc-800 px-6 flex items-center justify-between z-10">
